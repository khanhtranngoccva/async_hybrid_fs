...
//!
//! ## Error handling
//! - Since the pending I/O represents an atomic operation, EINTR handling is not built into these operations. However, more complex operations support EINTR handling.
//!
//! ## Memory safety
...
pub use crate::directory::DirEntry;
pub use crate::directory::ReadDir;
pub use crate::directory::ReadDirOwned;
pub use crate::directory::ReadDirOwnedWithParent;
pub use crate::directory::ReadDirWithParent;
...
pub use client::WriteResult;
pub use client::WritevResult;
pub use client::pending_io::PendingIo;
pub use default::default_client;
pub use default::default_client_owned;
...
pub trait HybridFile: UringTarget + Sync + Send {
    ...
    #[inline]
    fn vfs_metadata<'a>(&'a self) -> PendingIo<'a, io::Result<Statvfs>> {
        self.hybrid_vfs_metadata()
    }

    /// Alias for [`HybridFile::vfs_metadata`].
    fn hybrid_vfs_metadata<'a>(&'a self) -> PendingIo<'a, io::Result<Statvfs>> {
        default_client().fstatvfs(self)
    }
    ...
    {
        default_client().register_owned(self.into())
    }
}
...
pub trait HybridDir: UringTarget + Sync {
    ...
    /// This method is partially cancellation-safe. See [cancellation safety notes](`crate#cancellation-safety-and-correctness`) for details.
    #[inline]
    fn fchdir<'a>(&'a self) -> PendingIo<'a, io::Result<()>> {
        self.hybrid_fchdir()
    }

    /// Alias for [`HybridDir::fchdir`].
    ///
    /// # Cancellation safety
    /// This method is partially cancellation-safe. See [cancellation safety notes](`crate#cancellation-safety-and-correctness`) for details.
    fn hybrid_fchdir<'a>(&'a self) -> PendingIo<'a, io::Result<()>> {
        default_client().fchdir(self)
    }
}
...
impl HybridFile for Dir {}
impl HybridDir for Dir {}
